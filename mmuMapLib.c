...
/* Copyright (c) 1998, 2001, 2004, 2005, 2013 Wind River Systems, Inc.
 *
...
--------------------
01e,19aug13,zgl  LPAE support (WIND00357632)
01d,08feb05,rec  use VIRT_ADDR and PHYS_ADDR instead of void
01c,16jan04,scm  remove diab compiler warnings
...
VIRT_ADDR mmuPhysToVirt
	...
	if (((PHYS_ADDR)sysPhysMemDesc[i].physicalAddr <= physAddr) &&
	    (((UINT)sysPhysMemDesc[i].physicalAddr + sysPhysMemDesc[i].len - 1)
							     >= (UINT)physAddr))
	    return (VIRT_ADDR)((UINT)sysPhysMemDesc[i].virtualAddr +
		    (UINT)(physAddr - sysPhysMemDesc[i].physicalAddr));
	}

    /* Not really much to be done here, we found no match */

    return (VIRT_ADDR)physAddr;
    }
